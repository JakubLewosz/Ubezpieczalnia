...
- Poczta: `correspondence` jest częścią tego samego backendu. Klient IMAP tylko EXAMINE/UID/BODY.PEEK, bez mutacji u dostawcy; konfiguracja wyłącznie serwerowa. `fixtures/mail/*.eml` muszą mieć jawne DANE TESTOWE. Uruchom osobno worker `ocr` i worker `mail,maintenance,celery`; test prawdziwego lokalnego TLS opisuje docs/LOCAL_IMAP.md.
